...
#Shamelessly appropriated from https://github.com/glenpp/OrviboS20/blob/master/S20control.py
class orviboS20:
    port = 10000
...
'''
Small script hacked up in an afternoon or two to control a radiator in my room using an Orvibo S20 socket, a USB thermometer and a Raspberry Pi.
Repositories used:
...
while 1:
    try:
        ...
        if loops % 12 == 0:
            state = getState()
        temp = getTemperature()
        if temp < wantedTemp and state == 0:
